...
namespace UniGLTF
{
    public static class AnimationImporterUtil
    {
        ...
        public static IEnumerable<VRMShaders_new.SubAssetKey> EnumerateSubAssetKeys(glTF gltf)
        {
            foreach (var gltfAnimation in gltf.animations)
            {
                yield return new VRMShaders_new.SubAssetKey(typeof(AnimationClip), gltfAnimation.name);
            }
        }
    }
}